...
async function testNewWord(word) {
  const startTime = Date.now();

  try {
    const url = 'http://localhost:3000/api/tts?text=' + encodeURIComponent(word) + '&type=2';
...
async function runTests() {
  ...
  for (const word of testWords) {
    const result = await testNewWord(word);
