...
def backup_and_expand() -> Optional[Dict[str, Any]]:
    """
    主函数 - 备份并扩充数据库

    Returns:
        Dict: 更新后的数据库，失败时返回None

    注释说明:
    - 为什么要备份: 防止数据丢失，可回滚到之前版本
    - 边缘情况处理: 文件不存在、JSON格式错误、权限不足
    """
    try:
        # === 步骤1: 验证文件存在 ===
        # 防御性编程: 检查文件是否存在后再操作
        if not LIVESTREAM_DB.exists():
            print(f"[ERROR] Database file not found: {LIVESTREAM_DB}")
            print(f"[INFO] Please ensure the file exists before running this script")
            return None

        # === 步骤2: 读取现有数据库 ===
        # Context First: 先读取，不猜测数据结构
        print("="*70)
        ...
        try:
            with open(LIVESTREAM_DB, 'r', encoding='utf-8') as f:
                data = json.load(f)
            print(f"[OK] Loaded database: {LIVESTREAM_DB.name}")
        except json.JSONDecodeError as e:
            print(f"[ERROR] Invalid JSON format: {e}")
            print(f"[INFO] Please check if the file is corrupted")
            return None
        ...
        # 空值卫士: 检查数据有效性
        if not data:
            print("[WARNING] Empty database file")
            return None

        # === 步骤3: 备份数据库 ===
        # 注释: 为什么先备份 - 防止数据损坏后无法恢复
        print(f"\n[Backing up...]")
        try:
            # 确保备份目录存在
            ...
            print(f"[ERROR] Permission denied: Cannot write to {BACKUP_DB}")
            return None
        except Exception as e:
        ...
        if not existing_words:
            print("[WARNING] No existing words found in database")
            existing_words = []
        ...
        for category, word_list in WEEK3_VOCABULARY.items():
            ...
            for item in word_list:
                # 解包: 确保元组格式正确
                if len(item) != 4:
                    print(f"    - [ERROR] Invalid tuple format: {item}")
                    continue

                word, cn, level, example = item

                # 防御性编程: 检查必填字段
                if not all([word, cn, example]):
                    print(f"    - [WARNING] Missing required fields: {word}")
                    continue
                ...
                # 创建新词汇条目
                # 注释: 为什么使用replace和lower - 确保word_id格式统一
                try:
                    entry = {
                        "word": word,
                    ...
                    print(f"    - [ERROR] Failed to add '{word}': {e}")
                    continue

        # === 步骤6: 补充词汇以达到500词 ===
        # 注释: 为什么需要补充 - 确保精确达到500词目标
            ...
            for i, (word, cn, level, example, cat) in enumerate(supplementary[:remaining]):
                if word.lower() not in existing_word_set:
                    try:
                        ...
                        print(f"    - [ERROR] Failed to add supplementary '{word}': {e}")
                        continue
        ...
        # 更新历史记录（追加模式）
        if "expansion_history" not in data["meta"]:
            data["meta"]["expansion_history"] = []

        data["meta"]["expansion_history"].append({
            "wave": "week3",
            "date": datetime.now().isoformat(),
            "words_added": total_added,
            "words_skipped": total_skipped,
        ...
        try:
            ...
            print(f"[ERROR] Permission denied: Cannot write to {LIVESTREAM_DB}")
            return None
        except Exception as e:
        ...
        print(f"  Words skipped: {total_skipped}")
        ...
        if len(existing_words) >= 500:
            print(f"\n{'='*70}")
            print(" "*10 + "500 WORDS ACHIEVED! GOAL COMPLETED!")
            print("="*70)
...
def main() -> int:
    ...
    Returns:
        int: 0表示成功，非0表示失败
    """
    print("[Starting Week 3 Final Expansion]")
    print("Applying Secure & Consistent Coding Standards\n")

    final_data = backup_and_expand()

    if final_data is None:
        print("\n[FAILED] Expansion completed with errors")
        return 1

    print("\n[SUCCESS] All expansion weeks completed!")
    print(f"Database: {LIVESTREAM_DB.name}")
    print(f"Total words: {final_data['meta']['total_words']}")
    print(f"Goal achieved: {final_data['meta']['total_words'] >= 500}")
