...
async function importWordbooks() {
  ...
  for (const file of files) {
    ...
    try {
      ...
      // 1. 创建 book 记录
      const category = BOOK_CATEGORIES[bookTitle.split(' ')[0]] || 'exam'
      ...
      for (const word of data.words) {
        ...
        if (word.audio_url) fieldStats.has_audio_url++
        if (word.difficulty_score) fieldStats.has_difficulty_score++
  ...
  results.forEach(r => {
    ...
    console.log(`     audio_url:       ${((r.fieldStats.has_audio_url / r.totalWords) * 100).toFixed(1)}%`)
    console.log(`     difficulty_score:${((r.fieldStats.has_difficulty_score / r.totalWords) * 100).toFixed(1)}%`)
  })
