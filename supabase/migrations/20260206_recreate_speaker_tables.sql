...
-- 删除触发器（如果存在）
DROP TRIGGER IF EXISTS speaker_articles_updated_at ON speaker_articles;
DROP TRIGGER IF EXISTS speaker_progress_updated_at ON speaker_progress;

-- 删除函数（如果存在）
DROP FUNCTION IF EXISTS update_speaker_updated_at();

-- 删除表（注意顺序：先删有外键的表）
DROP TABLE IF EXISTS speaker_ghost_words CASCADE;
DROP TABLE IF EXISTS speaker_dictation_submissions CASCADE;
DROP TABLE IF EXISTS speaker_progress CASCADE;
DROP TABLE IF EXISTS speaker_sentences CASCADE;
DROP TABLE IF EXISTS speaker_articles CASCADE;
...
-- 添加索引
CREATE INDEX idx_speaker_articles_level ON speaker_articles(level);
CREATE INDEX idx_speaker_articles_status ON speaker_articles(status);

-- ========================================
-- 3. speaker_sentences - 句子表
-- ========================================
...
-- 添加索引
CREATE INDEX idx_speaker_sentences_article_id ON speaker_sentences(article_id);

-- ========================================
-- 4. speaker_progress - 学习进度表
-- ========================================
...
-- 添加索引
CREATE INDEX idx_speaker_progress_user_id ON speaker_progress(user_id);
CREATE INDEX idx_speaker_progress_article_id ON speaker_progress(article_id);
CREATE INDEX idx_speaker_progress_status ON speaker_progress(status);

-- ========================================
-- 5. speaker_dictation_submissions - 听写提交记录表
-- ========================================
...
-- 添加索引
CREATE INDEX idx_speaker_dictation_submissions_user_id ON speaker_dictation_submissions(user_id);
CREATE INDEX idx_speaker_dictation_submissions_article_id ON speaker_dictation_submissions(article_id);

-- ========================================
-- 6. speaker_ghost_words - 魔鬼生词本表
-- ========================================
...
-- 添加索引
CREATE INDEX idx_speaker_ghost_words_user_id ON speaker_ghost_words(user_id);
CREATE INDEX idx_speaker_ghost_words_article_id ON speaker_ghost_words(article_id);
CREATE INDEX idx_speaker_ghost_words_is_mastered ON speaker_ghost_words(is_mastered);
...
CREATE FUNCTION update_speaker_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
...
DO $$
BEGIN
  RAISE NOTICE '✅ 演说家模块数据库表创建成功！';
  RAISE NOTICE '已创建 5 个表：';
  RAISE NOTICE '  - speaker_articles (文章表)';
  RAISE NOTICE '  - speaker_sentences (句子表)';
  RAISE NOTICE '  - speaker_progress (进度表)';
  RAISE NOTICE '  - speaker_dictation_submissions (听写提交表)';
  RAISE NOTICE '  - speaker_ghost_words (生词本表)';
END $$;
