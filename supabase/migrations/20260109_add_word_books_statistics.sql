...
-- 添加注释
COMMENT ON COLUMN word_books.learner_count IS '学习该单词书的人数';
COMMENT ON COLUMN word_books.completion_rate IS '平均完成率（百分比0-100）';

-- 创建索引以加速按学习人数排序的查询
CREATE INDEX IF NOT EXISTS word_books_learner_count_idx
  ON word_books(learner_count DESC);

-- 创建索引以加速按完成率排序的查询
CREATE INDEX IF NOT EXISTS word_books_completion_rate_idx
  ON word_books(completion_rate DESC);
