...
const CARD_BASE_CLASS = 'group bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-150 overflow-hidden'

/** 单个播主卡片 */
function CreatorCard({ creator, className }: { creator: PodcastCreatorListItem; className: string }) {
...
export default function PodcastZone() {
  ...
  return (
    <section className="mb-10">
      ...
      {/* 移动端：水平滚动 */}
      <div
        className="md:hidden overflow-x-auto pb-3 -mx-4 px-4"
        ...
        <div className="flex gap-4" style={{ minWidth: 'min-content' }}>
          {creators.map((creator) => (
            <CreatorCard
              key={creator.id}
              creator={creator}
              className={`flex-shrink-0 w-[200px] ${CARD_BASE_CLASS}`}
            />
          ))}
        </div>
      </div>

      {/* PC端：网格布局，一行5个（xl），与视频列表同宽 */}
