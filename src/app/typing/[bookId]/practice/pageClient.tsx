...
// src/app/typing/[bookId]/practice/page.tsx
// 打字练习核心游戏页
...
import { ArrowLeft, Settings, Volume2, RotateCcw, SkipForward, CheckCircle2 } from 'lucide-react'
...
export default function TypingPracticePageClient() {
    ...
    const loadWords = async () => {
      try {
        ...
        if (scope === 'mistakes') {
          ...
          if (user) {
            ...
            if (wordIds.length > 0) {
              ...
              // 没有错题，返回空
              actions.startSession(bookId, scope, null, [])
              setLoading(false)
              return
            }
          }
          ...
          if (user) {
            if (scope === 'new') {
              // 未标注：从未练习过的单词
              // 🔥 修复：查询所有有进度记录的单词，然后排除
              const { data: practicedData } = await supabase
                .from('word_progress')
                .select('word_id')
                .eq('user_id', user.id)
                .eq('book_id', bookId)

              const practicedWordIds = practicedData?.map(p => p.word_id) || []
              if (practicedWordIds.length > 0) {
                query = query.not('id', 'in', `(${practicedWordIds.join(',')})`)
              }
            } else {
              // known, fuzzy, unknown：有对应状态的单词
              // 🔥 由于有UNIQUE约束，直接查询即可
        ...
        if (words && words.length > 0) {
          actions.startSession(bookId, scope, null, words)
        } else {
  ...
  const charStatuses = currentSession.charStatuses || []
          ...
          <div className="text-center mb-12">
            ...
            <div className="mb-8">
              <div
                ...
                style={{
                  fontSize: `${settings.foreignFontSize}px`,
